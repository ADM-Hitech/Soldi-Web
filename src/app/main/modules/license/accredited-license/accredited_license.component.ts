...
@Component({
  selector: 'app-accredited-license',
  templateUrl: './accredited_license.component.html',
  styleUrls: ['./accredited_license.component.scss'],
  animations: appAnimations,
  encapsulation: ViewEncapsulation.None
})
export class AccreditedLicenseComponent implements OnInit {
  ...
  fetchData(page: number = 1, numrecord: number = 20, filter: string = ''): void {
    ...
    this.rest.fetchTableAccredited(page, numrecord, filter, this.licenseId).subscribe((response) => {
      if (response.success) {
  ...
  delete(event): void {
    ...
    dialogRef.afterClosed().subscribe((response) => {
      if (response) {
        this.rest.deleteAccredited(event.id, event.type).subscribe(res => {
          if (res.success) {
